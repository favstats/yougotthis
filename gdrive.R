...
data_folder <- drive_ls(path = "ArnoldBot")
data_folder <- data_folder[data_folder$name != "img_links.rds",]
...
## download function ----
get_drive_data <- function(g_id, data_name) {
...
map2(data_folder$id, data_folder$name,
     ~ get_drive_data(g_id = .x, data_name = .y))
cat("\nAll done!\n")

newdata <- data_folder[order(data_folder$name),]

data_dat <<- data.frame(g_id = newdata$id,
       local = dir("data", full.names = T))
# 
# map2(newdata$id, dir("data", full.names = T),
