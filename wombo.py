...
tasks_url = "https://paint.api.wombo.ai/api/tasks/"
headers = {
...
def start_task(prompt, style):
    ...
    # get id of task
    tasks_response = requests.post(tasks_url, json=tasks_payload, headers=headers)
    ...
    # state is pending
    requests.put(url=tasks_url + task_id, headers=headers,
                 json=first_request_payload)
    time.sleep(1)

    # state is generating
    second_request = requests.get(url="https://paint.api.wombo.ai/api/tasks/" + task_id, headers=headers)
    task_state = json.loads(second_request.text)["state"]

    return task_id, task_state, first_request_payload

def get_task_state(task_id):

    # repeat second request until state is completed
    state_request = requests.get(url="https://paint.api.wombo.ai/api/tasks/" + task_id, headers=headers)
...
def get_final_result(task_id):
    ...
    final_request = requests.get(url="https://paint.api.wombo.ai/api/tasks/" + task_id, headers=headers)
